...
var Client = function (connectionString, clientId) {
    ...
    this.brokers = {} //address:port ==> socket
    this.topicMetadata = {};
    this.topicPartitions = {};
    this.correlationId = 0;
    this.cbqueue = {};
    this.brokerMetadata = {} //index ==> address:port
    this.ready = false;
    this.retryCnt = 0; //if got some error, should try get the metadata again, try several times
    this.connect();
}
util.inherits(Client, events.EventEmitter);

function getItemCount(object) {
    var num = 0;
    _.each(object, function(value1, key1) {
        ...
        decoder = protocol.decodeFetchResponse(function (err, type, value) {
            if (err) {
                ...
                if (self.retryCnt ++ > 2) {
                    ...
                    return consumer.emit('MetadataChanged');
                }
            }
            ...
            if (type === 'message') {
                ...
                _.each(value, function(value1, key1) {
                    topics[key1] = topics[key1] || {};
                    _.each(value1, function(value2, key2) {
                        topics[key1][key2] = value2;
                    });
                });
                if (getItemCount(topics) < payloads.length) return;
...
function wrap(payloads, cb) {
    ...
    return function (err, data) {
        ...
        _.each(data, function(value1, key1) {
            out[key1] = out[key1] || {};
            _.each(value1, function(value2, key2) {
                out[key1][key2] = value2;
            });
        });

        // Waiting for all request return
        if (getItemCount(out) < payloads.length) return;
        cb && cb(null, out);
    }
}
...
Client.prototype.createBroker = function connect(host, port) {
    ...
    socket.on('error', function (err) {
        //self.emit('error', err);
        if (_.isEmpty(self.brokerMetadata)) {//if the broker is empty, that means all broker is dead
            self.emit('error', 'broker connection got an error: ' + socket.addr + ' ' + err);
        } else {
            //console.log('broker connection got an error: ' + socket.addr + err);
            self.emit('MetadataChanged')
        }
    });
